...
def getwidth(depths: np.ndarray, energies: np.ndarray, range: float | None = None, sobp_width: float = 2) -> tuple:
    peak = energies.max()
    ...
    print(f"Entrance/peak: " f"{np.round(100 * np.average(energies[0:5]) / peak, 1)}%")
    ...
    if range:
        ...
        print(f"target stdev: " f"{np.round((target_stdev * 100) / np.average(target_dose), 3)}" f"%")
