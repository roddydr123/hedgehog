...
p = 1.8
alpha = 1.9E-3
SOBPwidth = 0.5     # set the desired SOBP width in cm
db = 4.0              # desired depth of distal edge in water
steps = 30           # desired no. blocks/weights
...
def blockHeight(steps=steps):
    return SOBPwidth / (steps * waterEquiv)


def wToEdges(weights):

    # changed to finding area of square rather than hexagon
    A_pinbase = d_across_pinbase**2
    ...
    # now the edge length is just the root of its area
    edges = np.sqrt(As)
